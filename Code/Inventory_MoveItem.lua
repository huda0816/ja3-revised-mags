...
function MoveItem(args)
	...
	if sync_call then
		NetUpdateHash("MoveItem", item and item.class, item and item.id,
			dest_container and type(dest_container) == "table" and dest_container.class or dest_container,
			dest_container_slot_name,
	...
	if dbgMoveItem then
		invprint("MoveItem sync", sync_call, "check_only", check_only, "amount", amount,
			string.format("moving %s %s from %s %s %d %d to %s %s %d %d, item_at_dest %s %s",
				item.class, tostring(item.id),
				src_container and src_container.session_id or src_container and src_container.class or false,
				src_container_slot_name, src_x or -1, src_y or -1, dest_container.session_id or dest_container.class,
				tostring(dest_container_slot_name), dest_x or -1, dest_y or -1,
				item_at_dest and item_at_dest.class or tostring(item_at_dest),
				item_at_dest and tostring(item_at_dest.id) or "n/a"))
	end
	if sync_call then
	...
	local is_combine = not (IsKindOf(dest_container, "Unit") and dest_container:IsDead() or IsKindOf(dest_container, "ItemContainer")) and
	MoveItem_CombinesItems and InventoryIsCombineTarget(item, item_at_dest)
	...
	if not sync_call then
		sync_ap, sync_unit = GetAPCostAndUnit(item, src_container, src_container_slot_name, dest_container,
		...
		if InventoryIsCombatMode() then
			...
			if not sync_unit:UIHasAP(sync_ap) then
				return is_reload and "Unit doesn't have ap to reload" or "Unit doesn't have ap to execute action", false,
					sync_unit
			end
		else
			sync_ap = 0
	...
	local function Sync()
		...
		if not sync_call then
			local args = MoveItem_SendNetArgs(item, src_container, src_container_slot_name, dest_container,
				dest_container_slot_name, dest_x, dest_y, amount, merge_up_to_amount, exec_locally,
	...
	if not item_at_dest then
		...
		if merge_stacks then
			local is_mergable, new_amount = MergeStackIntoContainer(dest_container, dest_container_slot_name, item,
				"check", merge_up_to_amount, local_stack_changes)
			if not is_mergable or new_amount > 0 then
				--no part of the stack is mergable or only part is mergable, check for free slot
				p_pos, reason = dest_container:CanAddItem(dest_container_slot_name, item, dest_x, dest_y,
					local_items_moved)
				if not p_pos then
					if not is_mergable then
						--no room and we can't merge any part of the stack
						return "move failed, no part of the stack is transferable and dest inventory refused item",
							reason, sync_unit
					else
	...
	if is_refill then
		...
		if usedmeds == allmedsNeeded then
			...
			item_at_dest.Condition = Clamp(
			item_at_dest.Condition + MulDivRound(usedmeds, max_condition - item_at_dest.Condition, allmedsNeeded), 0,
				max_condition)
		end
	...
	if alternative_pos then
		...
		local pos2, reason2 = src_container:AddItem(src_container_slot_name, item_at_dest, swap_src_x, src_y,
			is_local_changes, dest_x)
		assert(pos2, reason2)
	end
	if item_at_dest_2 then
