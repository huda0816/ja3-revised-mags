		...
		contextSubMenuReloadOptions.element.run_after = function(child, context, item, i, n, last)
			if IsKindOf(item.ammo, "Mag") then
				...
				child:SetText(T({
					433225540474,
					"<ammo_type>(<count>) Mag",
					ammo_type = item.ammo.ammo.DisplayName,
					count = count
				}))
...
-- 								local cost_ap, unit = GetAPCostAndUnit(context.item, context.context, ui_slot.slot_name,
-- 									dest_container, GetContainerInventorySlotName(dest_container))
